...
checkPresetProperties :: SpecWith Preset
checkPresetProperties = do
...
spec = do
  ...
  describe "presetToPng" $ do
    it "produces a valid PNG signature" $ do
      png <- runConduitRes $ do
        preset <- sourceFile "kpp/basic-shape-grainy.kpp" .| pngToPreset
        presetToPng preset .| sinkLazy
      png `shouldSatisfy` BL.isPrefixOf pngMagicString

    let withRoundtripPreset action = runConduitRes $ do
          preset <- sourceFile "kpp/basic-shape-grainy.kpp" .| pngToPreset
          tmpFile <- presetToPng preset .| sinkSystemTempFile "kpp-tool"
          liftIO $ loadPreset tmpFile >>= action

    aroundAll withRoundtripPreset
      checkPresetProperties
